...
namespace EpiRisk
{
  namespace Mcmc
  {
	...
	*proposal_ = *likelihood_;
    ...
    float
    LikelihoodHandler::ProposeNCInfecTimes(const float oldGamma, const float newGamma, const float power)
    {
      float ratio = powf(oldGamma/newGamma, power);
      proposal_->RescaleNCInfecTimes(ratio);
      return proposal_->CalcNCInfecTimes(power);
    }
    float
    LikelihoodHandler::GetNCInfecTimes(const float power) const
    {
      return likelihood_->CalcNCInfecTimes(power);
    }
