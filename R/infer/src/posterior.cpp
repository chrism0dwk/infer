...
void
postparams(H5::H5File& file, size_t burnin, size_t thin, Rcpp::DataFrame& posterior)
{
...
postinfecs(H5::H5File& file, size_t burnin, size_t thin, Rcpp::List& posterior)
{
  ...
  for(size_t i=0; i<numRecords; ++i)
      ...
      for(size_t j=0; j<buff.len; ++j)
	{
      ...
      free(buff.p);
      Rcpp::DataFrame valframe(Rcpp::DataFrame::create(Rcpp::Named("time")=val));
...
RcppExport SEXP readPosterior(SEXP filename, SEXP burnin, SEXP thin, SEXP actions)
{ 
  ...
  // Get parameters
  if(_actions[0] == 0) {
    Rcpp::DataFrame params; postparams(file, _burnin[0], _thin[0], params);
    Rcpp::List infecs; postinfecs(file, _burnin[0], _thin[0], infecs);
    return Rcpp::List::create(Rcpp::Named("parameters")=params, Rcpp::Named("infec")=infecs);
  } 
    ...
    ::Rf_error("Unknown action type specified");
  }
}
