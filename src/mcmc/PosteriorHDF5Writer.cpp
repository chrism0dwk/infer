...
namespace EpiRisk
{
  ...
  PosteriorHDF5Writer::PosteriorHDF5Writer(std::string filename, GpuLikelihood& likelihood) :
    ...
    if(isFirstWrite_)
      {
        ...
        paramTable_ = new FL_PacketTable(posterior.getId(), "parameters", paramRow_t.getId(), PARAMCHUNK);
        ...
        infecTable_ = new FL_PacketTable(posterior.getId(), "infections",infecRow_t.getId(),INFECCHUNK);
        ...
        H5::Attribute paramAttr(file_->openDataSet("/posterior/parameters").createAttribute("tags",
            paramTag_t,
            *paramAttrSpace));
  ...
  void
  PosteriorHDF5Writer::flush()
  {
    if(!isFirstWrite_) file_->flush(H5F_SCOPE_LOCAL);
  }
