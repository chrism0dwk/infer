/*************************************************************************
 *  ./src/unitTests/fmdSellkeDiagnostic.cpp
...
struct Settings
{
  ...
  load(const string& filename)
  {
    ...
    population = pt.get<string> ("fmdSellkeDiagnostic.paths.population");
    epidemic = pt.get<string> ("fmdSellkeDiagnostic.paths.epidemic");
    posterior = pt.get<string> ("fmdSellkeDiagnostic.paths.posterior");
    infecTimes = pt.get<string> ("fmdSellkeDiagnostic.paths.infecTimes");
    output = pt.get<string> ("fmdSellkeDiagnostic.paths.output");
    obstime = pt.get<double> ("fmdSellkeDiagnostic.options.obstime");
    skip = pt.get<size_t> ("fmdSellkeDiagnostic.options.skip");
  }
...
main(int argc, char* argv[])
{
  ...
  SellkeDiagnostic<FmdModel>* onestepahead = new SellkeDiagnostic<FmdModel>(model, settings.output);

  int lineCounter = 1;
  while(posterior.next())
    {
      if(lineCounter % settings.skip == 0) {
